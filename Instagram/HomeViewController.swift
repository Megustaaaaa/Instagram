...
class HomeViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, PostTableViewCellDelegate {
    ...
    var commentTappedCellIndex = 0
    
    
    // databaseのobseveEventの登録状態を表す
    var observing = false
    ...
    func commentButtonTapped(_ cell: PostTableViewCell) {
        if let row = cell.indexPath?.row {
            print("commentButtonTapped at : \(row)")
            commentTappedCellIndex = row
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if let vc = segue.destination as? CommentViewController {
            vc.postData = postArray[commentTappedCellIndex]
        }
    }
